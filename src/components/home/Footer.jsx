...
const Footer = () => {
    return (
      ...
      <div className=" text-black mx-auto w-full max-w-7xl px-8 pt-5">
        <div className="grid grid-cols-1 justify-between gap-4 md:grid-cols-2">
          <Typography variant="h1" className="mb-6 font-title text-md lg:text-lg xl:text-xl">
            GlobeXVAspire
          </Typography>
          <div id="contact" className="grid grid-cols-3 justify-between gap-4 font-detail">
            {LINKS.map(({ title, items }) => (
              <ul key={title}>
                <Typography
                  variant="small"
                  className="mb-3 text-md lg:text-lg xl:text-xl"
                >
                  {title}
                </Typography>
                {items.map((item) => (
